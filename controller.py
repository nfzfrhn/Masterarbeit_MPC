...
from uploadData_yearly_15minute import temperatureData, solarData, HeizWarmeBedarfData, COP_data, priceData
from progressBar_mpc import progressBar
...
def mpc_controller():
    ...
    query.next()
    num_bat = query.value(0)                            # Number of batteries
    print("num_bat: ", num_bat)
    # Initialize empty arrays for battery parameters
    batInit = cs.DM.zeros((num_bat,1))
    batEta = cs.DM.zeros((num_bat,1))
    batMinSOC = cs.DM.zeros((num_bat,1))
    batMaxSOC = cs.DM.zeros((num_bat,1))
    batMinPower = cs.DM.zeros((num_bat,1))
    batMaxPower = cs.DM.zeros((num_bat,1))

    # Get the battery parameters from the database
    bOk = query.exec_("SELECT * FROM Battery")
    if not bOk:
        qtw.QMessageBox.critical(None, "Error", query.lastError().text())
    ...
    batEtaIdx = batRecord.indexOf("batteryEfficiencyCoefficient")
    ...
    # Get the battery values from the database and store them in the arrays
    i = 0
    while query.next():
        batInit[i] = query.value(batInitIdx) * 1000
        batEta[i] = query.value(batEtaIdx)
        batMinSOC[i] = query.value(batMinEnergyIdx) * 1000
        batMaxSOC[i] = query.value(batMaxEnergyIdx) * 1000
    ...
    print("num_ev: ", num_ev)
    # Initialize empty arrays for EV parameters
    evInit = cs.DM.zeros((num_ev, 1))
    evEta = cs.DM.zeros((num_ev, 1))
    ...
    # Get the EV parameters from the database
    ...
    evEtaIdx = evRecord.indexOf("evEfficiencyCoefficient")
    ...
    # Get the EV values from the database and store them in the arrays
    i = 0
    while query.next():
        evInit[i] = query.value(evInitIdx)*1000
        evEta[i] = query.value(evEtaIdx)
    ...
    query.next()
    m = cs.DM(query.value(1))
    kwt = cs.DM(query.value(2))
    kt = cs.DM(query.value(3))
    mhp_dot = cs.DM(query.value(4))
    cwat = cs.DM(query.value(5))
    WP_Leistung_min = cs.DM(query.value(6)*1000)
    WP_Leistung_max = cs.DM(query.value(7)*1000)

    query.exec_("SELECT * FROM SolarPanel")
    query.next()
    solarModulType = query.value(1)     # Name of the PV module - This could be useful in the future
    n_PV_mod = cs.DM(query.value(2))    # Number of PV modules
    G_PV_NOCT = cs.DM(query.value(3))   # in 1 kW/m^2, STC reference solar irradiance
    T_PV_NOCT = cs.DM(query.value(4))   # in 1 °C, NOCT module temperature
    P_PV_STC = cs.DM(query.value(5))    # in 1 kW, STC power per module
    gamma_PV = cs.DM(query.value(6))    # in 1 / °C, STC temperature coefficient of module
    G_PV_STC = cs.DM(query.value(7))    # in 1 kW/m^2, STC reference solar irradiance
    T_PV_STC = cs.DM(query.value(8))    # in 1 °C, STC reference cell temperature

    # Get the Controller parameters from the database
    query.exec_("SELECT * FROM Controller")
    query.next()
    predictionHorizon = query.value(1)
    controlHorizon = query.value(2)
    T_mpc = query.value(3)*60                   # Sampling time in seconds
    sim_day = query.value(4)
    W1 = cs.DM(query.value(5))
    W2 = cs.DM(query.value(6))
    W3 = cs.DM(query.value(7))
    ...
    if ctrlMode == 1:
        ...
        print("Error: Invalid control mode selected")

    # MPC configuration
    T_sim = cs.DM(60)                       # This is to convert the sampling time from minutes to seconds
    # T_mpc = cs.DM(900)
    # sim_day = 6                             # TODO: replace with the one from OpenFileDialog
    daySec = 24 * 3600
    # sim_tim = sim_day * daySec
    N = int(predictionHorizon * 3600 / T_mpc)

    # weekly_ev = np.concatenate((idle_ev1, active_ev2, idle_ev3))
    # # time_ev = np.random.randint(0, 2, [num_ev, 4*24*sim_day])
    # time_ev = np.tile(weekly_ev, 52)        # Could be implemented for multiple EVs
    idle_ev = np.zeros(int(4 * 24 * 6 / 6))
    active_ev = np.ones(int(4 * 24 * 6 / 6))
    weekly_ev = np.concatenate((idle_ev, idle_ev, idle_ev, idle_ev, idle_ev))
    charging_ev = np.empty((0, 4*24*6))
    used_rd_nr = []
    for i in range(num_ev):
        rd_nr = rd.randint(1, 4)
        while rd_nr in used_rd_nr:
            rd_nr = rd.randint(1, 5)
        print(f"Random number: {rd_nr}")
        row = np.insert(weekly_ev, rd_nr*len(idle_ev), active_ev)
        charging_ev = np.vstack((charging_ev, row))

    time_ev = np.tile(charging_ev, (1, 52))
    soc_ev = np.zeros([num_ev, 4*24*sim_day])
        ...
        for j in range(1, 4*24*sim_day):
            ...
            if tick_ev == 1:
                ...
                for k in range(flag, j):
                    ...
                    soc_lower[i, k] = max(soc_ev[i, flag], soc_ev[i, j] + 1 / 24 * (k - j))
                    if soc_lower[i, k] > soc_upper[i, k]:
                        soc_lower[i, k] = soc_upper[i, k]

    evMaxEnergy = np.array(evMaxEnergy).reshape(-1, 1)
    #print(f"evMaxEnergy: {evMaxEnergy}")

    for i in range(num_ev):
        soc_ev[i, :] = evMaxEnergy[i] * soc_ev[i, :]
        soc_upper[i, :] = evMaxEnergy[i] * soc_upper[i, :]
        soc_lower[i, :] = evMaxEnergy[i] * soc_lower[i, :]
    ...
    # eta_Bat = cs.DM([0.96, 0.55])
    # n_Bat_mod = cs.DM(8)
    # E_Bat_nom = cs.DM(4000) * n_Bat_mod
    # E_Bat_min = cs.DM(0)
    # E_Bat_max = E_Bat_nom
    # p_Bat_max = n_Bat_mod*cs.DM(65*51.2*1000)
    # P_Bat_max = [8000, 10000]
    # P_Bat_min = np.negative(P_Bat_max)
    #batMinSOC = [1600, 2000]
    #batMaxSOC = [16000, 20000]

    # Electric Car - TODO:Implement using database
    # eta_EV = cs.DM([0.88, 0.95])
    # n_EV_mod = cs.DM(6)
    # E_EV_nom = cs.DM(3800) * n_EV_mod
    # E_EV_min = cs.DM(0)
    # E_EV_max = E_EV_nom
    # P_EV_max = cs.DM([8000, 10000])
    # ev_capacity = [48000, 40000]
    # soc_ev = ev_capacity * soc_ev
    # soc_ev[0, :] = ev_capacity[0] * soc_ev[0, :]
    # soc_ev[1, :] = ev_capacity[1] * soc_ev[1, :]
    #soc_upper = ev_capacity * soc_upper
    # soc_upper[0, :] = ev_capacity[0] * soc_upper[0, :]
    # soc_upper[1, :] = ev_capacity[1] * soc_upper[1, :]
    #soc_lower = ev_capacity * soc_lower
    # soc_lower[0, :] = ev_capacity[0] * soc_lower[0, :]
    # soc_lower[1, :] = ev_capacity[1] * soc_lower[1, :]
    ...
    def update_constraints(mpc_iter):
        startStateSliceLocal = int(T_mpc*controlHorizon*mpc_iter/T_mpc)
        stepStateStateSliceLocal = int(T_mpc/T_mpc)
        stopStateStateSliceLocal = startStateSliceLocal + int(N*T_mpc/T_mpc) + stepStateStateSliceLocal
        timeFrame_state = slice(startStateSliceLocal, stopStateStateSliceLocal, stepStateStateSliceLocal)

        startConSliceLocal = int(T_mpc * controlHorizon * mpc_iter / T_mpc)
        stepConSliceLocal = int(T_mpc / T_mpc)
        stopValConSliceLocal = startConSliceLocal + int((N - 1) * T_mpc / T_mpc) + stepConSliceLocal
        ...
        # index_offset_bat_st_local = n_states - num_ev
        index_offset_ev_st_local = n_states - num_ev

        # index_offset_bat_con_local = n_controls - num_bat - num_ev
        # index_offset_bat_con_local = n_controls - num_ev
        index_offset_ev_con_local = n_controls - num_ev
        ...
        try:
            # PV_used
            args['lbx'][n_states * (N + 1) + 1:n_states * (N + 1) + n_controls * N:n_controls] = 0
            args['ubx'][n_states * (N + 1) + 1:n_states * (N + 1) + n_controls * N:n_controls] = get_pv(timeFrame_control)
            ...
            # PV_sold
            args['lbx'][n_states*(N+1) + 4:n_states*(N+1) + n_controls*N:n_controls] = 0
            args['ubx'][n_states*(N+1) + 4:n_states*(N+1) + n_controls*N:n_controls] = get_pv(timeFrame_control)
            ...
            for i in range(index_offset_ev_con_local, index_offset_ev_con_local + num_ev):
                ...
                args['ubx'][n_states*(N+1) + i:n_states*(N+1) + n_controls*N:n_controls] = evMaxPower[index_tracking_ev_con]*time_ev[index_tracking_ev_con, timeFrame_ev_control]        # Current cost
                # args['ubx'][n_states*(N+1) + i:n_states*(N+1) + n_controls*N:n_controls] = 8000*time_ev[timeFrame_ev_control]                               # Current cost
            ...
            print(f"The error was at mpciter:{mpc_iter} and (control). length get_time_ev:{len(time_ev[index_tracking_ev_con, timeFrame_ev_control])}")

        try:
            args['lbg'][n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_control) + 0.0001
            args['ubg'][n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_control) + 0.0002
            args['lbg'][n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_control)+ 0.0001
            args['ubg'][n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_control)+ 0.0002
        except:
    ...
    def updateEV_State(mpc_iter, stEV):
        ...
        index_updateEV_State = n_states - num_ev
        for kEV in range(num_ev):
            dtEV = time_ev[kEV, controlHorizon*mpc_iter] - time_ev[kEV, controlHorizon*mpc_iter-1]
            if dtEV == 1:
                st_ev = soc_ev[kEV, controlHorizon*mpc_iter]
                #stEV=cs.vertcat(stEV[:-1], st_ev)
                stEV[index_updateEV_State+kEV] = st_ev
            elif dtEV == -1:
                st_ev = soc_ev[kEV, controlHorizon*mpc_iter]
                # stEV=cs.vertcat(stEV[:-1], cs.DM(0))
                stEV[index_updateEV_State + kEV] = cs.DM(0)
        return stEV
    ...
    for i in range(num_bat):
        ...
        rhs = cs.vertcat(rhs, (batEta[i] ** cs.sign(p_bat[i]) * p_bat[i] / 3600))

    # RHS - Electric Car
    for j in range(num_ev):
        # rhs = cs.vertcat(rhs, (eta_EV ** cs.sign(p_ev) * p_ev / T_mpc))
        rhs = cs.vertcat(rhs, (evEta[j] * p_ev[j] / 3600))
    ...
    for k in range(N):
        ...
        if k != N:
            obj = obj + W1*con[7]*(con[0]-con[1]-y_current[0]+J_bat+J_ev)*((con[0]-con[1]-y_current[0]+J_bat+J_ev)>0) + W2*(con[0]-con[1]-y_current[0]+J_bat+J_ev)*((con[0]-con[1]-y_current[0]+J_bat+J_ev)<0) \
                  + W3*(con[0]-con[1]-y_current[0]+J_bat+J_ev)**2
        ...
        l2 = cs.vertcat(l2,
                        ...
                        con[0]*(con[0]>WP_Leistung_min),                                # Limit power of heat pump with binary var phi
                        con[0]*(con[0]<WP_Leistung_min))                                # Limit power of heat pump with binary var phi
        # l2 = cs.vertcat(l2,
        #                 con[2] + con[4],                                                # Total PV power
        #                 con[2] + y_current[0],                                          # Total PV power
    ...
    lbg = cs.DM.zeros((n_states*(N+1) + n_outputs*N + 4*N), 1)
    ubg = cs.DM.zeros((n_states*(N+1) + n_outputs*N + 4*N), 1)
    ...
    startPV = 0
    stepPV = int(T_mpc/T_mpc)
    stopPV = int((N-1) * T_mpc / T_mpc) + stepPV
    timeFrame_PV = slice(startPV, stopPV, stepPV)
    lbg[n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_PV) + 0.0001   # Correct value
    ubg[n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_PV) + 0.0002   # Correct value
    lbg[n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_PV) + 0.0001   # Correct value
    ubg[n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 4*N: 4] = get_pv(timeFrame_PV) + 0.0002   # Correct value
    lbg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 4*N: 4] = 0
    ubg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 4*N: 4] = WP_Leistung_max
    lbg[n_states*(N+1) + n_outputs*N + 3: n_states*(N+1) + n_outputs*N + 4*N: 4] = 0
    ubg[n_states*(N+1) + n_outputs*N + 3: n_states*(N+1) + n_outputs*N + 4*N: 4] = 0
    ...
    # State constraint
    startStateSlice = 0
    stepStateSlice = int(T_mpc / T_mpc)
    stopStateSlice = int(N * T_mpc / T_mpc) + stepStateSlice
    ...
    index_SOC_bat_constraint = 0
    # index_battery_st = 11
    for i in range(index_offset_bat_st, index_offset_bat_st + num_bat):
        if num_bat >= 1:
            # TODO: Implement database functionality
            lbx[i:n_states * (N+1): n_states] = batMinSOC[index_SOC_bat_constraint]
            ubx[i:n_states * (N+1): n_states] = batMaxSOC[index_SOC_bat_constraint]
            index_offset_ev_st = i + 1
            index_SOC_bat_constraint = index_SOC_bat_constraint + 1

    index_SOC_EV_constraint = 0
    for i in range(index_offset_ev_st, index_offset_ev_st + num_ev):
        if num_ev >= 1:
            # TODO: Implement database functionality
            lbx[i:n_states * (N+1): n_states] = soc_lower[index_SOC_EV_constraint,:N+1]        # Is this N+1 correct?
            ubx[i:n_states * (N+1): n_states] = soc_upper[index_SOC_EV_constraint,:N+1]        # Is this N+1 correct?
            index_tracking_ev_st = index_SOC_EV_constraint + 1
    ...
    ubx[n_states*(N+1) + 1: n_states*(N+1) + n_controls*N: n_controls] = get_pv(timeFrame_PV) + 0.0002

    # mt_dot - control constraint
    lbx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = mt_dot_min
    ubx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = mt_dot_max

    # Toa - control constraint
    startConSlice = 0
    stepConSlice = int(T_mpc/T_mpc)
    stopConSlice = int((N-1) * T_mpc / T_mpc) + stepConSlice
    ...
    ubx[n_states*(N+1) + 4: n_states*(N+1) + n_controls*N: n_controls] = get_pv(timeFrame_PV) + 0.0002
    ...
    for k in range(index_offset_battery_con, index_offset_battery_con + num_bat):
        if num_bat >= kk:
            lbx[n_states * (N + 1) + k: n_states * (N + 1) + n_controls * N: n_controls] = batMinPower[index_P_bat_constraint]
            ubx[n_states * (N + 1) + k: n_states * (N + 1) + n_controls * N: n_controls] = batMaxPower[index_P_bat_constraint]
            index_offset_ev_con = k + 1
            index_P_bat_constraint = index_P_bat_constraint + 1

    # Power input EV - control constraint
    index_P_EV_constraint = 0
    ll = 1
    for l in range(index_offset_ev_con, index_offset_ev_con + num_ev):
        if num_ev >= ll:
            # lbx[n_states * (N + 1) + l: n_states * (N + 1) + n_controls * N: n_controls] = evMinPower[index_P_EV_constraint]
            lbx[n_states * (N + 1) + l: n_states * (N + 1) + n_controls * N: n_controls] = 0
            ubx[n_states * (N + 1) + l: n_states * (N + 1) + n_controls * N: n_controls] = evMaxPower[index_P_EV_constraint]
    ...
    state_init = [init_T1, init_T2, init_T3, init_T4, 0]
    ...
    # TODO: Can np.zeros and casadi variables working together? No, they can't
    u0 = cs.DM.zeros(n_controls, int(N))                            # n_controls*N
    # u0 = cs.DM()
    X0 = cs.repmat(x0, 1, N+1)  # This cs.repmat().T exist. I checked
    Y0 = cs.DM.zeros(n_outputs, int(N))                             # n_outputs*N
    ...
    cat_controls = DM2Arr(u0[:, 0].T).reshape(1, -1)
    ...
    pBar.show()
    total_iteration = (sim_day-1)*96
    t_start_sim = time.time()
    while mpciter < total_iteration:                 # Max is 481 for 6 day and 577 for 7 days. Originally was sim_tim / T_mpc
        t1 = time.time()
        u0[:, -1] = 0
        u0[0, :] = 8000
        args['p'] = cs.vertcat(
        ...
        progressValue = int(mpciter / total_iteration * 100)
        pBar.update_progress(progressValue)


    t_end_sim = time.time()
    print("Simulation time is {}".format(t_end_sim - t_start_sim))
    #pBar
    return xx, yy, cat_controls, lbg, ubg, lbx, ubx, t, times, soc_lower, soc_upper, num_bat, num_ev
