...
pv_power = n_PV_mod * P_PV_STC * (pv_power_raw / G_PV_STC) * (1 + gamma_PV * (T_PV_mod - T_PV_STC))  # in 1 kW, PV power
...
def update_constraints(mpc_iter):
    ...
    try:
        args['lbx'][n_states*(N+1) + 3:n_states*(N+1) + n_controls*N:n_controls] = get_temp_of_air(timeFrame_local)-0.002   # Toa
        args['ubx'][n_states*(N+1) + 3:n_states*(N+1) + n_controls*N:n_controls] = get_temp_of_air(timeFrame_local)+0.002   # Toa
    except:
    ...
    try:
        args['lbg'][n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 2*N: 2] = get_pv(timeFrame_local)-0.001
        args['ubg'][n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 2*N: 2] = get_pv(timeFrame_local)+0.001
    except:
...
ctrls = cs.vertcat(p_bought, pv_used, mt_dot, toa, pv_sold, p_bat, p_ev)
...
# php = p_bought + pv_used - cs.sum1(p_bat)
php = p_bought * (p_bought > 600)
...
for k in range(N):
    ...
    l2 = cs.vertcat(l2,
                    con[0]*(con[0]>600),                                            #
                    con[1] + y_current[3],                                          # PV power balance constraint
                    con[0]*(con[0]<600),                                            #
                    con[0] - con[1] + J_bat + J_ev,                                 # Power Balance Constraint
                    con[1] - J_bat)                                                 # Use PV first before using battery
    st_next_RK4 = st
...
# Constraint function
lbg = cs.DM.zeros((n_states*(N+1) + n_outputs*N + 5*N), 1)
ubg = cs.DM.zeros((n_states*(N+1) + n_outputs*N + 5*N), 1)

# lbg[start:stop:step]
# ubg[start:stop:step]

# Net power - This implementation is correct. This includes offset(offset is 0), length
# p_bought*p_bought>600
lbg[n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 5*N:5] = 600                  # Correct value
ubg[n_states*(N+1) + n_outputs*N + 0: n_states*(N+1) + n_outputs*N + 5*N:5] = 4800               # Correct value
...
lbg[n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 5*N: 5] = get_pv(timeFrame_PV) + 0.0001   # Correct value
ubg[n_states*(N+1) + n_outputs*N + 1: n_states*(N+1) + n_outputs*N + 5*N: 5] = get_pv(timeFrame_PV) + 0.0002   # Correct value
...
# Power from grid
# lbg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 5*N:5] = 0                  # p_bought*(p_bought<600)
# ubg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 5*N:5] = 0                  # p_bought*(p_bought<600)
lbg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 5*N:5] = 0                  # p_bought*(p_bought<600)
ubg[n_states*(N+1) + n_outputs*N + 2: n_states*(N+1) + n_outputs*N + 5*N:5] = 600                  # p_bought*(p_bought<600)
...
for j in range(index_offset_ev, index_offset_ev + num_ev):
    if num_ev >= 1:
        ...
        lbx[j:n_states * (N+1): n_states] = 0
        ubx[j:n_states * (N+1): n_states] = 12000
        index_tracking_st = j
...
# mt_dot
lbx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = 0.0124
ubx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = 0.124
# lbx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = 111111
# ubx[n_states*(N+1) + 2: n_states*(N+1) + n_controls*N: n_controls] = 111111
...
lbx[n_states*(N+1) + 3: n_states*(N+1) + n_controls*N: n_controls] = get_temp_of_air(timeFrame_Toa) + 0.0001
ubx[n_states*(N+1) + 3: n_states*(N+1) + n_controls*N: n_controls] = get_temp_of_air(timeFrame_Toa) + 0.0002
...
for l in range(index_offset_ev_con, index_offset_ev_con + num_ev):
    if num_ev >= ll:
        lbx[n_states * (N + 1) + l: n_states * (N + 1) + n_controls * N: n_controls] = -8000
...
# Compute
state_init = [12, 15, 15, 25, 35, 35, 35, 35, 1, 0]
...
ax1.plot(t, cat_controls[:, 0], label="p_bought")   # P_bought
ax1.plot(t, cat_controls[:, 1], label="PV_used")    # PV_used
ax1.plot(t, cat_controls[:, 4], label="PV_sold")    # PV_sold
ax1.legend()
ax2.plot(t, cat_controls[:, 3], label="Toa")        # Toa
...
ax5.legend()
ax6.plot(t, cat_controls[:, 5], label="P_Battery")  # Battery
ax6.plot(t, cat_controls[:, 6], label="P_EV")       # EV
plt.show()
