...
def mpc_controller():
    ...
    # Initialization
    # TODO: Later uncomment this and search for num_bat and num_ev
    #num_bat = 1
    #num_ev = 1
    ...
    #t_temperature, temperature_outside, pv_power_raw, heizbedarf,  = prognoseData15()
    ...
    # idle_ev1 = np.zeros(int(4*24/3))
    # active_ev2 = np.ones(int(4*24/3))
    # idle_ev3 = np.zeros(int(4*24/3))
    idle_ev1 = np.zeros(int(4 * 24 * 6/ 3))
    active_ev2 = np.ones(int(4 * 24 * 6/ 3))
    idle_ev3 = np.zeros(int(4 * 24 * 6/ 3))
    # daily_ev = np.concatenate((idle_ev1, active_ev2, idle_ev3))
    weekly_ev = np.concatenate((idle_ev1, active_ev2, idle_ev3))
    # time_ev = np.tile(weekly_ev, 365)
    time_ev = np.tile(weekly_ev, 52)
    # soc_ev = np.zeros([num_ev, 4*24*sim_day+1])
    # soc_upper = np.zeros([num_ev, 4*24*sim_day])
    # soc_lower = np.zeros([num_ev, 4*24*sim_day])
    soc_ev = np.zeros(4*24*365)
    soc_upper = np.zeros(4*24*365)
    soc_lower = np.zeros(4*24*365)

    flag = 0
    for j in range(1, 4 * 24 * sim_day):
        tick_ev = time_ev[j] - time_ev[j - 1]
        if tick_ev == 1:
            soc_ev[j] = round(rd.uniform(0, 1), 2)
            flag = j
        elif tick_ev == -1:
            soc_ev[j] = soc_ev[flag] + 0.7 * round(rd.uniform(0, 1), 2) + 0.3
            if soc_ev[j] > 1 or soc_ev[j] > 1 / 24 * (j - flag) + soc_ev[flag]:
                soc_ev[j] = min(1, 1 / 24 * (j - flag) + soc_ev[flag])
            for k in range(flag, j):
                soc_upper[k] = min(1, soc_ev[flag] + 1 / 24 * (k - flag))
                soc_lower[k] = max(soc_ev[flag], soc_ev[j] + 1 / 24 * (k - j))
                if soc_lower[k] > soc_upper[k]:
                    soc_lower[k] = soc_upper[k]
    ...
    def update_constraints(mpc_iter):
        ...
        try:
            # T1
            args['lbx'][0:n_states * (N + 1):n_states] = 45-30/21*(get_temp_of_air(timeFrame_state))
            # T2
            ...
            # SOC_EV
            args['lbx'][6:n_states * (N + 1):n_states] = soc_lower[timeFrame_ev_state]
            args['ubx'][6:n_states * (N + 1):n_states] = soc_upper[timeFrame_ev_state]
        ...
        try:
            ...
            # P_ev
            args['lbx'][n_states*(N+1) + 9:n_states*(N+1) + n_controls*N:n_controls] = 0
            args['ubx'][n_states*(N+1) + 9:n_states*(N+1) + n_controls*N:n_controls] = 8000 * time_ev[timeFrame_ev_control]  # Current cost
    ...
    def updateEV_State(mpc_iter, stEV):
        ...
        dtEV = time_ev[controlHorizon*mpc_iter] - time_ev[controlHorizon*mpc_iter-1]
        if dtEV == 1:
            st_ev = soc_ev[controlHorizon*mpc_iter]
            stEV=cs.vertcat(stEV[:-1], st_ev)
        elif dtEV == -1:
            st_ev = soc_ev[controlHorizon*mpc_iter]
            stEV=cs.vertcat(stEV[:-1], cs.DM(0))
    ...
    SOC_bat = cs.SX.sym("SOC_bat")
    SOC_ev = cs.SX.sym("SOC_ev")
    ...
    rhs = cs.vertcat((kt*(t2-t1) - kwt*(t1-toa) + mt_dot*cwat*(t2-t1)) / (m * cwat),
            ...
            cs.SX(1),
            eta_Bat ** cs.sign(p_bat) * p_bat / 3600,
            eta_EV * p_ev / 3600)
    ...
    for k in range(N):
        ...
        if k != N:
            ...
            obj = obj + 10*con[7]*(con[0]-con[1]-y_current[0]+con[8]+con[9])*((con[0]-con[1]-y_current[0]+con[8]+con[9])>0)+0.06 * (con[0]-con[1]-y_current[0]+con[8]+con[9])*((con[0]-con[1]-y_current[0]+con[8]+con[9])<0) \
                    + (con[0]-con[1]-y_current[0]+con[8]+con[9])**2
                # + 0.5*J_bat**2 + (st[5]-cs.DM(32000))**2
        ...
        l2 = cs.vertcat(l2,
                        con[1] + con[4],                                                # Total PV power
                        con[1] + y_current[0],                                          # Total PV power
    ...
    # TODO: In Non-Modular form
    # SOC Battery - state constraint
    lbx[5:n_states * (N + 1): n_states] = 1200
    ubx[5:n_states * (N + 1): n_states] = 12000

    # TODO: In Non-Modular form
    # SOC EV - state constraint
    lbx[6:n_states * (N + 1): n_states] = soc_lower[0:N+1]  # Is this N+1 correct?
    ubx[6:n_states * (N + 1): n_states] = soc_upper[0:N+1]  # Is this N+1 correct?
    ...
    ubx[n_states*(N+1) + 4: n_states*(N+1) + n_controls*N: n_controls] = WP_Leistung_max
    ...
    lbx[n_states*(N+1) + 8: n_states * (N + 1) + n_controls * N: n_controls] = -8000
    ubx[n_states*(N+1) + 8: n_states * (N + 1) + n_controls * N: n_controls] = 8000
    ...
    lbx[n_states*(N+1) + 9: n_states * (N + 1) + n_controls * N: n_controls] = 0
    ubx[n_states*(N+1) + 9: n_states * (N + 1) + n_controls * N: n_controls] = 8000
    # ll = 1
    ...
    # state_init = [50, 50, 50, 50, 0, 10000, 0]
    state_init = [50, 50, 50, 50, 0, 2000, 0]
    ...
    # u0 = cs.DM.zeros(int(N), n_controls)                                    # N*n_controls TODO: Change to n_controls*N
    u0 = cs.DM.zeros(n_controls, int(N))                                      # n_controls*N
    # X0 = cs.repmat(x0, 1, N).T  # This cs.repmat().T exist. I checked
    X0 = cs.repmat(x0, 1, N+1)  # This cs.repmat().T exist. I checked         # n_states*(N+1)
    # Y0 = cs.DM.zeros(int(N), n_outputs)                                     # N*n_outputs TODO: Change to n_outputs*N
    ...
    # cat_controls = DM2Arr(u0[0, :]).reshape(1, -1)                          # TODO: Adapt to changes in u0
    ...
    while mpciter < 480:                 # Max is 481 for 6 day and 577 for 7 days. Originally was sim_tim / T_mpc
        t1 = time.time()
        u0[:,-1] = 0                      # Why this 0 necessary? Does the zero affect P_ev?
        u0[0,:] = 8000
        args['p'] = cs.vertcat(
            x0,  # Current state
            xs  # Target state
...
if __name__ == "__main__":
    statesValue, output, control, lbg, ubg, lbx, ubx, t, times = mpc_controller()
    np.savetxt('log_xx_array.txt', statesValue, fmt='%.5f')
    ...
    ax3.plot(t, statesValue[:, 0], label="T1")               # T1
    ax3.plot(t, statesValue[:, 1], label="T2")               # T2
    ax3.plot(t, statesValue[:, 2], label="T3")               # T3
    ax3.plot(t, statesValue[:, 3], label="T4")               # T4
    ax3.set_ylabel("Temperature [°C]")
    ax3.legend()
    ax4.plot(t, statesValue[:, 5], label="SOC Battery")      # SOC Battery
    ax4.plot(t, statesValue[:, 6], label="SOC EV")           # SOC EV
