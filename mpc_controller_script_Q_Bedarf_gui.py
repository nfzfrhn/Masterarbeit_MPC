...
def mpc_controller():
    ...
    for i in range(num_bat):
        ...
        rhs = cs.vertcat(rhs, (eta_Bat ** cs.sign(p_bat[i]) * p_bat[i] / 3600))

    # RHS - Electric Car
    for j in range(num_ev):
        # TODO: Implement database functionality
        # rhs = cs.vertcat(rhs, (eta_EV ** cs.sign(p_ev) * p_ev / T_mpc))
        rhs = cs.vertcat(rhs, (eta_EV * p_ev[j] / 3600))
    ...
    for i in range(index_offset_bat_st, index_offset_bat_st + num_bat):
        if num_bat >= 1:
            # TODO: Implement database functionality
            lbx[i:n_states * (N+1): n_states] = 1200
            ubx[i:n_states * (N+1): n_states] = 12000
