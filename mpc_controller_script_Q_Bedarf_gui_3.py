...
# from dummy_uploadData_yearly import temperatureData, solarData, HeizWarmeBedarfData, COP_data, priceData
...
def mpc_controller():
    ...
    G_PV_NOCT = cs.DM(query.value(3))   # in 1 W/m^2, STC reference solar irradiance
    T_PV_NOCT = cs.DM(query.value(4))   # in 1 °C, NOCT module temperature
    P_PV_STC = cs.DM(query.value(5))    # in 1 W, STC power per module
    gamma_PV = cs.DM(query.value(6))    # in 1 / °C, STC temperature coefficient of module
    G_PV_STC = cs.DM(query.value(7))    # in 1 W/m^2, STC reference solar irradiance
    ...
    t_ev = np.arange(4*24*365+1)
    # idle_ev1 = np.zeros(int(4 * 24 * 6 / 3))
    # active_ev2 = np.ones(int(4 * 24 * 6 / 3))
    # idle_ev3 = np.zeros(int(4 * 24 * 6 / 3))
    ...
    # weekly_ev1 = np.concatenate((idle_ev, active_ev, idle_ev, idle_ev, idle_ev, idle_ev))
    # weekly_ev2 = np.concatenate((idle_ev, idle_ev, idle_ev, idle_ev, active_ev, idle_ev))
    # time_ev1 = np.tile(weekly_ev1, 52)
    # time_ev2 = np.tile(weekly_ev2, 52)
    # time_ev = np.zeros((num_ev, 4 * 24 * 6 * 52))
    # time_ev[0, :] = time_ev1
    # time_ev[1, :] = time_ev2
    ...
    for i in range(num_bat):
        ...
        # rhs = cs.vertcat(rhs, (eta_Bat[i] ** cs.sign(p_bat[i]) * p_bat[i] / 3600))
    ...
    while mpciter < 200:                 # Max is 481 for 6 day and 577 for 7 days. Originally was sim_tim / T_mpc
        t1 = time.time()
        u0[:, -1] = 0
        u0[0, :] = 8000
        args['p'] = cs.vertcat(
    ...
    return xx, yy, cat_controls, lbg, ubg, lbx, ubx, t, times, soc_lower, soc_upper
...
if __name__ == "__main__":
    states, output, control, lbg, ubg, lbx, ubx, t, times, soc_lower, soc_upper = mpc_controller()
