...
def mpc_controller():
    ...
    while mpciter < 5:                 # Max is 576. Originally was sim_tim / T_mpc
        t1 = time.time()
...
if __name__ == "__main__":
    ...
    ax1.plot(t, control[:, 0], label="p_hp")       # P_hp
    ax1.plot(t, control[:, 1], label="p_bought")   # P_bought
    ax1.plot(t, control[:, 2], label="PV_used")    # PV_used
    ax1.plot(t, control[:, 5], label="PV_sold")    # PV_sold
    ax1.set_ylabel("Power [W]")
    ax1.legend()
    ax2.plot(t, control[:, 4], label="Toa")        # Toa
    ax2.plot(t, states[:, 0], label="Tw")   # Tw
    ax2.plot(t, states[:, 1], label="Tz")   # Tz
    ax2.set_ylabel("Temperature [°C]")
    ax2.legend()
    ax4.plot(t, states[:, 2], label="Tf")   # Tf
    ax4.plot(t, states[:, 3], label="Tpip") # Tpip
    ax3.plot(t, states[:, 4], label="T1")   # T1
    ax3.plot(t, states[:, 5], label="T2")   # T2
    ax3.plot(t, states[:, 6], label="T3")   # T3
    ax3.plot(t, states[:, 7], label="T4")   # T4
    ax3.set_ylabel("Temperature [°C]")
    ax4.plot(t, output[:, 2], label="Te")   # Te
    ax3.legend()
    ax4.plot(t, states[:, 8], label="COP")   # COP
    ax4.legend()
    ax5.plot(t, states[:, 10], label="SOC_Bat")   # SOC_Car
    ax5.plot(t, states[:, 11], label="SOC_EV")  # SOC_EV
    ax5.set_ylabel("SOC [%]")
    ax5.legend()
    ax6.plot(t, control[:, 6], label="P_Battery")  # Battery
    ax6.plot(t, control[:, 7], label="P_EV")       # EV
