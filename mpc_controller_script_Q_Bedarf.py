...
def mpc_controller():
    ...
    SOC_Bat_max = [16000, 20000]
    SOC_Bat_min = [1600, 2000]
    ...
    def update_constraints(mpc_iter):
        ...
        try:
            ...
            for i in range(index_offset_ev_con_local, index_offset_ev_con_local + num_ev):
                ...
                args['ubx'][n_states*(N+1) + i:n_states*(N+1) + n_controls*N:n_controls] = P_EV_max[index_tracking_ev_con]*time_ev[index_tracking_ev_con, timeFrame_ev_control]        # Current cost
    ...
    for j in range(num_ev):
        ...
        rhs = cs.vertcat(rhs, (eta_EV[j] * p_ev[j] / 3600))
    ...
    for k in range(N):
        ...
        if k != N:
            obj = obj + 10*con[7]*(con[0]-con[1]-y_current[0]+J_bat+J_ev)*((con[0]-con[1]-y_current[0]+J_bat+J_ev)>0) + 0.06*(con[0]-con[1]-y_current[0]+J_bat+J_ev)*((con[0]-con[1]-y_current[0]+J_bat+J_ev)<0) \
                  + (con[0]-con[1]-y_current[0]+J_bat+J_ev)**2
    ...
    for i in range(index_offset_bat_st, index_offset_bat_st + num_bat):
        if num_bat >= 1:
            # TODO: Implement database functionality
            lbx[i:n_states * (N+1): n_states] = SOC_Bat_min[index_SOC_bat_constraint]
            ubx[i:n_states * (N+1): n_states] = SOC_Bat_max[index_SOC_bat_constraint]
    ...
    for k in range(index_offset_battery_con, index_offset_battery_con + num_bat):
        if num_bat >= kk:
            lbx[n_states * (N + 1) + k: n_states * (N + 1) + n_controls * N: n_controls] = P_Bat_min[index_P_bat_constraint]
            ubx[n_states * (N + 1) + k: n_states * (N + 1) + n_controls * N: n_controls] = P_Bat_max[index_P_bat_constraint]
    ...
    for l in range(index_offset_ev_con, index_offset_ev_con + num_ev):
        if num_ev >= ll:
            ...
            ubx[n_states * (N + 1) + l: n_states * (N + 1) + n_controls * N: n_controls] = P_EV_max[index_P_EV_constraint]
    ...
    while mpciter < 481:                 # Max is 481 for 6 day and 577 for 7 days. Originally was sim_tim / T_mpc
        t1 = time.time()
        u0[:, -1] = 0
        u0[0, :] = 8000
        args['p'] = cs.vertcat(
